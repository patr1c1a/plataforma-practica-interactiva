...
def multiplication(nums):
    """
    Returns the multiplication of all numbers in a list. The list will only contain numbers.
    Example:
        multiplication(nums=[1, 2, 3, 4]) -> 24
    -Parameters:
        -nums (list, elements: numeric): List of numbers to be multiplied.
    -Returns:
...
def greatest_element(strings):
    ...
    -Parameters:
        -strings (list; elements: str): List of strings to search for the greatest element.
    -Returns:
...
def highest_profit(prices):
    ...
    sold on the day with the highest price. The list is not in chronological order and shares can be bought and sold
    on any two days. The list will have at least two elements.
    Example:
...
def no_duplicates(might_have_duplicates):
    """
    Returns a list of elements that appear only once in the given list.
    Examples:
    ...
    -Returns:
        (list; elements: multiple types) A new list with the elements from `might_have_duplicates` that are unique
        (have no duplicates).
    """
    pass
...
def percentages_even_odd_numbers(nums):
    ...
    -Returns:
        (tuple; elements: float). A 2-element tuple where its first element represents the percentage (from 0 to 100)
        of even numbers contained in `nums`, while the second element represents the percentage of odd numbers in
        `nums`.
    """
    pass
...
def missing_numbers(nums):
    ...
    -Returns:
        (list; elements: int) A new list where elements are the numbers from 0 to `n-1` that are not included in
        `nums`.
    """
    pass
...
def how_many_are_smaller(nums):
    ...
    Example:
        ...
        `i=1` and `i=2` store number 3 and there is one smaller element: 2. `i=3` stores 4 and there are three smaller 
        elements: 3, 3, 2. And for `i=4`, which stores number 2, there are no smaller elements).
    -Parameters:
        -nums (list; elements: numeric): List with numbers.
    -Returns:
...
def two_largest(nums):
    """
    Returns the two largest numbers in `nums`. The list contains at least two elements.
    Example:
        two_largest(nums=[5, 3, 6, 2, 8]) -> (8, 6)
    -Parameter:
        -nums (list; elements: numeric): List with numbers. len(nums) >= 2
    -Returns:
...
def discard_excess_occurrences(nums, maximum):
    """
    Given a list `nums` and a `maximum`, returns a new list with the elements from `nums` without exceeding a `maximum`
    of occurrences. That means that, for each element in `nums`, if it occurs more than `maximum` times, only up to
    `maximum` occurrences will be included in the new list. All other elements will preserve the same relative order
    they have in the `nums` list.
    Examples:
    ...
    -Returns:
        (list; elements: int) A new list where elements from `nums` are copied without altering their relative
        position, but discarding any occurrences that exceed the `maximum` amount allowed.
    """
    pass
...
def add_every_nth(nums, n):
    ...
    -Returns:
        (int) Sum of elements in `nums`, starting with the nth element and going forward every `n` elements. 0 if `n`
...
def move_zeroes(nums):
    ...
    elements. This will be done in-place, without using additional lists.
    Examples:
    ...
    -Parameter:
        -nums (list; elements: int): List with numbers.
    -Returns:
        (list; elements: int) The list (`nums`) where all zeroes have been moved to the end of the list and all
        non-zero elements keep their same relative position.
    """
    pass
...
def unnest(nested_lists):
    """
    Given a list whose elements are also (nested) lists, returns a new list where one level of nesting has been
    removed.
...
def how_many_passed(exam_results):
    ...
    Example:
        ...
        -> 3
    -Parameter:
        -exam_results (list; elements: lists containing 3 elements: str, int, int): List with student data in nested
        lists.
    -Returns:
...
def diagonal_sum(matrix):
    """
    Given a square matrix (made up of a nested lists), returns the sum of its main diagonal.
    Example:
    ...
    -Parameter:
        -matrix (list; elements: lists containing numeric elements): List with nested lists forming a matrix of `n`
        columns and `n` rows.
    -Returns:
        (numeric) Sum of the matrix main diagonal.
    """
    pass
...
def find_country(cities, city_name):
    """
    Given a list with tuples containing a city name and its country, and a city name, decides to which country the
    given city belongs. If the city is not included in any of the tuples, returns None.
    Example:
        ...
        -> "Scotland"
    -Parameters:
        -cities (list; elements: tuples containing 2 elements: str, str): List with cities and their matching
        countries, represented by tuples with the following structure: (city name, country name).
        -city_name (str): Name of a city.
    -Returns:
        (str / None) Name of the country that `city_name` belongs to. None if `city_name` is not found in any tuple
        within the cities list.
    """
    pass
...
def find_destination(tickets, cities, ticket_number):
    ...
    The ticket list includes, for each ticket: the ticket number and destination city. The cities list includes, for
    each city: the city name and the country it belongs to. The destination city can be calculated using
    `ticket_number`, and the destination country can be calculated using the city name.
