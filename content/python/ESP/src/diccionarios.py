...
def hallar_repetidos(strings1, strings2):
    """
    Dadas dos listas de strings (`strings1` y `strings2`), retorna cuáles de esos strings están incluidos en ambas
    listas.
...
def solo_una_mascota(perros, gatos):
    """
    Dadas dos listas: `perros` y `gatos`, conteniendo una los nombres de personas cuya mascota es un perro y, la otra,
    los nombres de personas cuya mascota es un gato, retorna cuáles de esas personas tiene únicamente perro o 
    únicamente gato, pero no ambos. Un mismo nombre puede estar en ambas listas, en caso de que una persona tenga ambas
    mascotas).
    Sugerencia didáctica: evitar iterar por las listas para lograr el cometido.
    Ejemplo:
    ...
    -Valor retornado:
        (set; elementos: str) Los nombres de aquellas personas que solo tienen perros o solo tienen gatos pero no
        ambos.
    """
    pass
...
def elementos_unicos(tuplas):
    """
    Dada una cantidad indeterminada de `tuplas` que contienen números, retorna una tupla que combina todas las
...
def listar_apellidos(alumnos):
    """
    Dada una lista `alumnos`, conteniendo los nombres y apellidos de los alumnos de una institución, retorna una 
    estructura que contiene los apellidos únicamente, sin repeticiones. Los nombres y apellidos de los alumnos están
    contenidos en un único string, separados por un espacio. Cada alumno puede tener más de un nombre pero solo un 
    apellido, en la forma: "nombre(s) apellido".
    Ejemplo:
...
def agregar_pelicula(peliculas, pelicula):
    """
    Agrega datos de una `pelicula` a un diccionario (`peliculas`) y lo retorna modificado. Si la película ya existe en
    el diccionario, la reemplaza con los nuevos datos. Los datos que se almacenan de cada película son: nombre, 
    director, año de estreno. El diccionario usa el nombre de cada película como claves y una lista con el resto de 
    datos como valor.
    Ejemplo:
    ...
    -Valor retornado:
        (dict; clave: str; valor: list, con 2 elementos: str, int) El diccionario `peliculas` con el nuevo dato 
        agregado.
    """
    pass
...
def domicilios_facturacion(ventas):
    """
    Dada la lista `ventas` que contiene tuplas con la información de cada venta realizada a clientes durante un mes,
    retorna los domicilios de cada cliente al cual se le debe enviar una factura de compra. Cada tupla de la lista 
    incluye, en este orden: nombre del cliente, día del mes, monto pagado y domicilio del cliente.
    Cada cliente puede haber hecho más de una compra en el mes, por lo que cada domicilio debe aparecer una sola vez.
    Ejemplo:
...
def mas_votados(votos, curso):
    """
    Dado un diccionario `votos` conteniendo los votos que se hicieron para seleccionar al "mejor compañero" en los 
    cursos de una escuela, y dado un número de `curso`, retorna los nombres de todos los alumnos que fueron votados, 
    sin repeticiones. El diccionario tiene como claves los números de los cursos y como valores listas de strings con 
    cada voto. Si el número de curso dado no se encuentra en el diccionario, retorna un conjunto vacío.
    Ejemplo:
    ...
    -Valor retornado:
        (set; elementos: str) Conjunto con los nombres de los alumnos votados en `curso`.
    """
    pass
...
def ocurrencias_digitos(digitos):
    """
    Dada una lista `digitos` que contiene dígitos numéricos, informa la cantidad de ocurrencias de cada dígito, 
    indicando el valor 0 para los dígitos (entre el 0 y el 9) que no se encuentran en la lista.
...
def mayor_valor(ocurrencias):
    """
    Dado un diccionario `ocurrencias` donde las claves son strings y los valores son números positivos únicos, retorna
    cuál es la clave que corresponde al mayor valor. Si el diccionario está vacío, retorna un string vacío. Cada valor
    solo ocurre una vez en el diccionario.
    ...
    -Valor retornado:
        (str) Clave que tiene asociado el mayor valor, o string vacío si el diccionario está vacío.
    """
    pass
...
def epoca_de_siembra(vegetales, mes):
    """
    Dado un diccionario `vegetales` conteniendo los meses de siembra de diversos vegetales, y el nombre de un `mes`,
    retorna qué vegetales pueden sembrarse en ese mes. La lista retornada conserva el orden en que los vegetales 
    aparecen como claves en el diccionario.
    Ejemplo:
...
def invertir_diccionario(diccionario):
    """
    Dado un `diccionario` donde las claves son strings y los valores son strings, retorna un nuevo diccionario donde
    las claves pasan a ser los valores y los valores pasan a ser las claves. Si un mismo valor aparece más de una vez
    en el diccionario original, en el resultado ese valor deberá asociarse a una lista con todas las claves que lo
    tenían.
    Ejemplos:
    ...
    -Valor retornado:
        ...
        `diccionario` y los valores son las claves de `diccionario`.
    """
    pass
...
def asentar_pago(socios, numero):
    """
    Dado un diccionario con la información de `socios` de un club y el `numero` de un socio, modifica el diccionario
    para registrar que ese socio tiene los pagos de la cuota social al día. El diccionario tiene como claves los
    números de socio y, como valores, listas con los datos del socio: [nombre, teléfono, estado de pagos (True si están
    al día, False en caso contrario)].
    Ejemplo:
    ...
    -Valor retornado:
        (dict; clave: int; valor: list, con 3 elementos: str, int, bool) Diccionario `socios` en el cual se ha
...
def eliminar_socio(socios, nombre_socio):
    """
    Dado un diccionario con la información de `socios` de un club y `nombre_socio` representando el nombre de un socio,
    lo elimina del diccionario. El diccionario tiene como claves los números de socio y, como valores, listas con los 
    datos de ese socio: [nombre, teléfono, estado de pagos (True si están al día, False en caso contrario)]. Si el 
    nombre dado no corresponde a ningún socio, el diccionario no se modifica. 
    Ejemplo:
    ...
    -Valor retornado:
        ...
        el socio con nombre `nombre_socio`, en caso de existir.
    """
    pass
...
def agrupar_por_longitud(cadenas):
    """
    Dada `cadenas` como una lista de strings, los agrupa según su longitud. El resultado debe ser un diccionario donde:
...
def romano_a_arabigo(romano):
    """
    Dado `romano` que es un número expresado en notación romana, retorna su equivalente en números arábigos en base
    decimal (usando reglas simplificadas).
...
def numero_telefonico(telefono):
    """
    Dado un número de `telefono` que contiene letras, lo retorna con sus equivalentes en números. El número dado solo
    contendrá números, letras mayúsculas, guiones "-" y paréntesis "(" y ")". Solo se convertirán las letras, dejando
    los números y otros símbolos sin modificación.
...
def cadenas_isomorficas(cadena1, cadena2):
    ...
    reemplazarse cada carácter de uno para obtener el otro. Todas las ocurrencias de un carácter deben ser reemplazadas
    con otro carácter, conservando su orden. No puede haber dos caracteres que sean reemplazados por el mismo carácter,
    pero sí es válido que un carácter se reemplace a sí mismo.
    Se asume que ambos strings tienen igual longitud y están compuestos por caracteres ascii válidos.
    Ejemplos:
...
def patron_de_palabras(patron, palabras):
    """
    Dado un `patron` y un string compuesto por `palabras`, indica si el string sigue el patrón, de manera que
    ...
    -Parámetros:
        ...
        al inicio ni al final del string. `palabras` puede contener solo letras minúsculas y el carácter ' '.
    -Valor retornado:
